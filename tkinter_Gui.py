...
def check_tie():
    ...
    if len(stop_list)==0 and '-' not in board :# len()==0 means stop not in list or '-' not in board means all the
        # spaces are filled
...
def update_board(position,button):
    ...
    if turn%2==0:
        ...
        if board[position-1] == '-': # condition for checking the position by player is empty or not. '-' refers to empty
            ...
            if 'stop' in stop_list : # Whenever player wins the check for winner function will add stop in empty list
                msg.showinfo('Winner','Congratulations....Player 1 wins.\n Well played ')
                print(turn)
                clear_list_and_reset_button_text()
            elif 'tie' in stop_list: # Whenever match is tie the chek for tie function will add tie in the empty list
                msg.showinfo('Tie','Ohh!....The match is tie')
                clear_list_and_reset_button_text()
            else: # If nothing happens then turning the player turns
                stop_list.clear()
                turn += 1
    ...
    # Odd refers for second player
        ...
        if board[position-1]=='-':# condition for checking the position by player is empty or not. '-' refers to empty
            ...
            if 'stop' in stop_list :
                msg.showinfo('info', 'Player 2 is winner')
                clear_list_and_reset_button_text()
                turn+=1
            elif 'tie' in stop_list:
                msg.showinfo('Tie', 'Ohh!....The match is tie')
                clear_list_and_reset_button_text()
            else: # if nothing happens then changing the turn by making even to odd or vice versa
                stop_list.clear()
                turn += 1
        else:
            msg.showinfo('Eroor!',"Please don't try to overite the position..")
